...
export function useMapTiler(props) {
    ...
    // Oppdater ref uten re-render
    useEffect(() => {
        activeLocationRef.current = activeLocation;
    }, [activeLocation]);

    /**
     * Rapporterer kartets status tilbake til systemet.
     */
    const reportMapStatus = useCallback(() => {
        const map = mapInstanceRef.current;
        if (!map || !map.isStyleLoaded()) return;

        // Tving MarkerLayout til å skanne utsnittet før vi trekker ut byer
    ...
    useEffect(() => {
        ...
        map.on("load", () => {
            ...
            // Overstyrer stilen så by-data lastes uavhengig av zoom
    ...
    }, []); // Tom array er SSOT for å unngå blinking
    ...
    useEffect(() => {
        ...
        if (geometryBounds) {
            // PRIORITET 1: Nøyaktig polygon (Bruker minZoom for å sikre by-visning)
